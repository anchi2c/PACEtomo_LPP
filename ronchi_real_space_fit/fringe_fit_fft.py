#!/usr/env/bin python
"""
This is extracted from leginon library for ronchigram fringe fitting.
It include a fourier space peak finding similar to that used in ronchi_lib.py but auto scale to handler larger range of c3 off-plane condition.
The resulting image_rotation is fed into a real-space fitting that is
better at estimate phase and fringe wavelength.
Leginon dependencies are stripped, and mrcfile is used to read/write mrc files if needed.
"""
import numpy
...
def _get_rough_angle_period(img_clipped, binning, number_of_peaks, number_of_fringe_guess):
    ...
    if binning == 1:
        ...
        import ronchi_lib
        img_binned = ronchi_lib._ronchi_bin_image(img_clipped, binning)
        if __name__ == "__main__":
